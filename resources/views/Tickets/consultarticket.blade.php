                    ...
                    @foreach ($tickets as $ticket)
                        <tr>
                            ...
                            <td >
                                ...
                                @elseif ($ticket->REINCIDENCIA == 'SI' || $ticket->AREA_RESPONSABLE == 'SEDENA' || $ticket->AREA_RESPONSABLE == 'SEMAR' || $ticket->DAÑO == 'Siniestro')
...
@section('js')
    ...
    <script>
        $('#detalleModal').on('show.bs.modal', function (event) {
            ...
            var source = `{!! asset('storage/tickets/evidencias/${ticket.FOTO_OBLIGATORIA}') !!}`;
            modal.find('.modal-body img#myimage_1').attr('src', source);

            var source = `{!! asset('storage/tickets/evidencias/${ticket.FOTO_2}') !!}`;
            modal.find('.modal-body img#myimage_2').attr('src', source);

            var source = `{!! asset('storage/tickets/evidencias/${ticket.FOTO_3}') !!}`;
